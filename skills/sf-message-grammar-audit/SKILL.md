...
description: "Audit every user-facing message in a Salesforce managed package for grammatical issues, then generate a prioritized remediation report. Extracts messages from Apex classes, Apex triggers, Lightning Web Components (LWC), Aura Lightning Components, Visualforce Pages, and Visualforce Components — the same extraction as sf-message-catalog — then runs each message through grammar analysis to flag article errors, subject-verb agreement issues, plural/singular mismatches, spelling mistakes, capitalization inconsistencies, punctuation problems, and cross-component wording inconsistencies. Produces a .docx report with current message, issue description, suggested fix, component reference, and priority. Use this skill when the user mentions 'grammar audit', 'message quality', 'fix error messages', 'grammatical issues', 'proofread messages', 'message consistency', 'message grammar check', 'clean up error messages', 'improve error messages', 'message quality report', or asks to review messages for grammar, spelling, or consistency. Also triggers on 'are there typos in our messages', 'check message wording', or 'message style review'. This skill is internal-facing — it helps development teams improve message quality before releases. It can be scheduled to run monthly."
---
